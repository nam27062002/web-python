...
from .models import Profile, Post, LikePost, FollowersCount, Comment, Chat, SavePost
    ...
    if request.method == 'POST':
        ...
        if username2 != "Default": 
            if username2 not in list_nickname:
                ...
                list_id = [list_id[index]] + \
                    list_id[:index] + list_id[index+1:]
                list_nickname = [list_nickname[index]] + \
...
def save_post(request):
    if request.method == 'POST':
        ...
        save_filter = SavePost.objects.filter(
            post_id=post_id, username=username).first()
        if save_filter == None:
            new_save = SavePost.objects.create(
                post_id=post_id, username=username)
            new_save.save()
            return JsonResponse({})
        else:
...
def upload(request):
    if request.method == 'POST':
        ...
        new_post = Post.objects.create(
            user=user, image=file_image, caption=caption)
        new_post.save()
        return redirect('/')
    else:
        return redirect('/')
    ...
    if request.method == 'POST':
        ...
        if User.objects.filter(email=data['email']).exists():
            ...
            new_profile = Profile.objects.create(
                user=user_model, id_user=user_model.id, full_name=data['fullname'])
            new_profile.save()
            return redirect('/')
    else:
...
def get_list_post(request):
    ...
    feed_list.sort(key=lambda x: int(x.created_at.timestamp()),
                   reverse=True)  # sắp xếp theo thời gian
    list_image_avatar = []
    list_nickname = []
    list_created_at = []
    list_image_post = []
    ...
    for post in feed_list:
        ...
        like_filter = LikePost.objects.filter(
            post_id=post.id, username=request.user.username).first()
        if like_filter == None:
        ...
        save_filter = SavePost.objects.filter(
            post_id=post.id, username=request.user.username).first()
        if save_filter == None:
            list_saved.append(False)
        else:
            list_saved.append(True)
    ...
    return {"list_image_avatar": list_image_avatar, "list_nickname": list_nickname, "list_created_at": list_created_at, "list_liked": list_liked, "list_saved": list_saved, "list_count_like": list_count_like, "list_image_post": list_image_post, "list_caption": list_caption, "list_post_id": list_post_id, "comments": comments, "list_count_cmt": list_count_cmt}
...
def get_profile(request, username):
    ...
    postType = request.GET.get('postType')
    ...
    if postType == "UserPost":
        user_posts = Post.objects.filter(
            user=user_view.username).order_by('-created_at')
    else :
        save_posts = SavePost.objects.filter(
             username=user_logined)
        if not save_posts:
            user_posts = save_posts
        else :
            user_posts = []
            for i in save_posts:
                
                post = Post.objects.filter(id = i.post_id)
                user_posts.append(post)
            user_posts = list(chain(*user_posts))  # list post
            user_posts.sort(key=lambda x: int(x.created_at.timestamp()),
                    reverse=True)
...
def getPost(request, postId):
    username = request.user.username
    status_like = len(LikePost.objects.filter(
        Q(post_id=postId) & Q(username=username))) > 0
    save_filter = SavePost.objects.filter(post_id=postId, username=username).first()
    if save_filter == None:
        status_save = False
    else:
        status_save = True
    ...
    try:
        if request.method == 'POST':
            ...
            if not User.objects.filter(email=email).first():
                messages.success(
                    request, 'Không tìm thấy địa chỉ email của bạn.')
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if new_password != confirm_password:
                messages.success(
                    request, 'Mật khẩu xác nhận không trùng khớp!')
                return redirect(f'/change-password/{token}/')
